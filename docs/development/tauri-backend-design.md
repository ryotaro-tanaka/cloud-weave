...
| **Orchestration**  | Command bodies, threading, progress polling, mapping to user-visible results      | Same crate, preferably **factored out** of the shell over time (e.g. `[unified_library.rs](../../src-tauri/src/unified_library.rs)` for unified library listing / load stream) |
