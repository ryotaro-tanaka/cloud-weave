...
fn start_auth_flow(
  ...
  match child.try_wait() {
    Ok(Some(_)) => match collect_child_output(child) {
      Ok(stdout) => {
        let record = success_auth_session(remote_name, provider, mode, &success_message(&stdout));
    ...
    Ok(None) => {
      ...
      spawn_auth_watcher(app.clone(), child, remote_name.to_string(), provider.to_string(), mode.to_string());
      ...
      Ok(CreateRemoteResult {
        ...
        status: "pending".to_string(),
        next_step: "open_browser".to_string(),
        message: "Authentication started in your browser. Return here after you finish signing in."
          .to_string(),
      })
    }
    Err(error) => {
      ...
      match child.try_wait() {
        Ok(Some(_)) => {
          match collect_child_output(child) {
            ...
            Err(error) => {
              if let Ok(result) = build_auth_error_result(&app, &remote_name, &provider, &mode, &error) {
...
fn build_auth_error_result(
  ...
  let result = match classify_rclone_error(error) {
    ...
    RcloneErrorKind::AuthCancelled => CreateRemoteResult {
      ...
      message: "Authentication was not completed. You can try again when you are ready.".to_string(),
    },
    ...
    match child.try_wait() {
      ...
      Ok(None) => {
        if start.elapsed() >= timeout {
          kill_child(&mut child);
          return Err(format!("operation timed out after {} seconds", timeout.as_secs()));
        }
      }
      Err(error) => {
        kill_child(&mut child);
...
fn parse_listremotes(raw: &str) -> Result<Vec<String>, String> {
  ...
  let remotes = match parsed {
    ...
    ListRemotesJson::Records(records) => records.into_iter().map(|record| record.name).collect(),
  };
...
fn pending_auth_session(remote_name: &str, provider: &str, mode: &str, message: &str) -> AuthSessionRecord {
  AuthSessionRecord {
    remote_name: remote_name.to_string(),
    provider: provider.to_string(),
    mode: mode.to_string(),
...
fn success_auth_session(remote_name: &str, provider: &str, mode: &str, message: &str) -> AuthSessionRecord {
  AuthSessionRecord {
    remote_name: remote_name.to_string(),
    provider: provider.to_string(),
    mode: mode.to_string(),
...
fn error_auth_session(remote_name: &str, provider: &str, mode: &str, message: &str) -> AuthSessionRecord {
  AuthSessionRecord {
    remote_name: remote_name.to_string(),
    provider: provider.to_string(),
    mode: mode.to_string(),
...
mod tests {
  ...
  fn parse_provider_map_reads_types_from_config() {
    ...
    assert_eq!(parsed.get("onedrive-main").map(String::as_str), Some("onedrive"));
    assert_eq!(parsed.get("dropbox-backup").map(String::as_str), Some("dropbox"));
  }
